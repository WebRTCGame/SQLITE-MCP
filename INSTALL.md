...
- If tools are still unavailable, fully reload or restart VS Code and start a new Agent chat session.
- If the server is not running after restart, run `MCP: Start Server` from the Command Palette and select `sqlite-project-memory`, then restart chat.
...
If tools still do not appear after all file-based gates pass, fully reload or restart VS Code and start a new Agent chat session.
If the server is not running after restart, run `MCP: Start Server` and select `sqlite-project-memory`, then restart the chat session.

Note: after MCP is used, VS Code/Copilot may create `.vscode/settings.json` with a `chat.mcp.serverSampling` entry for `sqlite-project-memory`. That file is generated by the editor, not by the installer, and is expected.
